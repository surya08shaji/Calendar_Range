...
class CalendarPicker(context: Context, attrs: AttributeSet) : LinearLayout(context, attrs) {
    ...
    private var currentYear = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
        Year.now().value
    } else {
                ...
                if (secondSelectedDate[YEAR_KEY] != null) {
                    ...
                    layers?.let {
                        highlightedDatesBackgroundColor?.let { color ->
                            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                                it.getDrawable(it.findIndexByLayerId(R.id.background))
                                    .setTint(color)
                            }
                        }
                        selectedDateCircleColor?.let { color ->
                        ...
                        if (tvsWithEvent.contains(tv.text.toString())) {
                            eventDotColor?.let { color ->
                                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                                    it.getDrawable(it.findIndexByLayerId(R.id.eventDot))
                                        .setTint(color)
                                }
                            }
                            eventDotColorWhenSelected?.let { color ->
                ...
                if (fromDate.length == 10) {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                        try {
                            ...
                            Log.e(
                                TAG,
                                e.localizedMessage ?: "error" + "/n" + e.stackTrace.toString()
                            )
                            // false
...
fun Long.toLocalDateTime(): LocalDateTime? {
    return try {
        val instant: Instant = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
